...
class JIDUtils {
    ...
    extractJIDs(message) {
        try {
            ...
            // Normalize to phone number format for consistency
            let chat_jid = remoteJid;
            
            // Convert business account LID to phone format if available
            if (remoteJid.endsWith('@lid') && key.senderPn) {
                chat_jid = key.senderPn;
            } else if (remoteJid.endsWith('@lid') && key.participantPn) {
                chat_jid = key.participantPn;
            }
    ...
    /**
     * Extract phone number from business account message key
     */
    extractPhoneFromBusinessAccount(key) {
        // Try various fields that might contain the phone number
        if (key.senderPn) return key.senderPn;
        if (key.participantPn) return key.participantPn;
        if (key.participant && !key.participant.endsWith('@lid')) return key.participant;
        
        // If we have the bot's own number and this is a business chat
        if (global.botJid) {
            return global.botJid;
        }
