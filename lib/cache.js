...
class CacheManager {
    ...
    startPeriodicTasks() {
        ...
        // Smart memory cleanup every 30 minutes
        setInterval(() => {
    ...
    smartMemoryCleanup() {
        try {
            ...
            // Smart cleanup: Clear portions since we run every 30 minutes now
            this.aggressiveCleanCache('message', 0.4);  // 40% every 30min
            
            // Clear 25% of user cache (keep useful user data longer)  
            this.aggressiveCleanCache('user', 0.25);
            
            // Clear 30% of group cache
            this.aggressiveCleanCache('group', 0.3);
            
            // Clear 60% of media cache (media still takes most memory)
            this.aggressiveCleanCache('media', 0.6);
            
            // Clear 20% of general cache
            this.aggressiveCleanCache('general', 0.2);
