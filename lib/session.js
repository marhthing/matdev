...
class SessionManager {
     ...
     * Backup session data
     */
    async backupSession(sessionId = 'default') {
        try {
            const sessionPath = this.getSessionPath(sessionId);
            ...
            if (await fs.pathExists(sessionPath)) {
                ...
                this.logger.success(`💾 Session backup created: ${path.basename(backupPath)}`);
                return backupPath;
            }
            ...
            this.logger.error(`Failed to backup session ${sessionId}:`, error);
            throw error;
        }
    }
     ...
     */
    async restoreSession(backupPath, sessionId = 'default') {
        try {
            await this.acquireLock(sessionId);
            ...
            this.logger.success(`♻️ Session restored from backup: ${sessionId}`);
