...
class MessageHandler {
    ...
    extractMessageInfo(message) {
        try {
            ...
            // Fix participant detection for outgoing messages from owner
            let participant;
            if (message.key.fromMe) {
                // For outgoing messages, the actual sender is the bot/owner
                const botNumber = this.bot?.sock?.user?.id?.split(':')[0] || config.OWNER_NUMBER;
                participant = `${botNumber}@s.whatsapp.net`;
            } else {
                // For incoming messages, handle business accounts (@lid) properly
                this.logger.info(`🔍 Debug - senderPn: ${message.key.senderPn}, sender: ${sender}, endsWith @lid: ${sender.endsWith('@lid')}`);
                if (message.key.senderPn && sender.endsWith('@lid')) {
                    // Business account - use the actual phone number
                    this.logger.info(`🔧 Business account detected! Using senderPn: ${message.key.senderPn} instead of remoteJid: ${sender}`);
                    participant = message.key.senderPn;
                } else if (isGroup) {
                    // Group message - use participant
                    participant = message.key.participant;
                } else {
                    // Regular private message - use sender
                    participant = sender;
                }
            }
