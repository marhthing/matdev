...
class MessageHandler {
    ...
    async process(message) {
        try {
            ...
            // Parse command first, then log (using processed commandText)
    ...
    async processStickerCommand(message, jids) {
        try {
            ...
            // Get sticker hash for identification
            const stickerHash = stickerMessage.fileSha256 ? 
            ...
            // Check if this sticker has a bound command
            const stickerCommands = this.database.getData('stickerCommands') || {};
            const boundCommand = stickerCommands[stickerHash];
            ...
            const messageKey = `${message.key?.id || Date.now()}:${stickerHash}:${boundCommand.command}`;
            const now = Date.now();
            ...
            console.log(`🎭 Sticker command triggered: ${boundCommand.command}`);
