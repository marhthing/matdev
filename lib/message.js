        ...
        try {
            ...
            if (messageTypes.includes('editedMessage')) {
                ...
                if (editedContent) {
                    ...
                    if (!contextInfo) {
                        console.log('🔍 Searching for contextInfo in all message layers...');
                        
                        // Check if there's a saved contextInfo from the original message that was edited
                        // We need to look for it in the database or reconstruct from available data
                        ...
                        if (messageKey && this.database) {
                            ...
                            if (archivedMessage && archivedMessage.contextInfo) {
                                ...
                                console.log('🔗 Restored contextInfo from archived message');
                            }
                        }
                        
                        // If still no contextInfo, check if we can find the quoted message in recent messages
                        // This is a fallback for when editing loses the reply context
                        if (!contextInfo) {
                            console.log('❌ No contextInfo found in any layer - reply context lost during editing');
                        }
                    }
                    ...
                    if (contextInfo) {
                        ...
                        // Try to find contextInfo from the original message before editing
                        console.log('🔍 Attempting to restore contextInfo from original message...');
                        try {
                            const messageKey = message.key?.id;
                            if (messageKey && this.database) {
                                // Look for a recent message with similar timestamp but without editing
                                ...
                                for (const recentMsg of recentMessages) {
                                    ...
                                    if (timeDiff < timeWindow && recentMsg.contextInfo && recentMsg.id !== messageKey) {
                                        try {
                                            ...
                                            console.log('🔗 Restored contextInfo from nearby message');
                                            break;
                                        } catch (parseError) {
                            ...
                            console.log('⚠️ Error restoring contextInfo:', error.message);
                        }
