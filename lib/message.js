...
class MessageHandler {
    ...
    async reply(messageInfo, text, options = {}) {
        try {
            ...
            // For bot=owner setup, always reply to the owner (participant), not remoteJid
            const replyJid = messageInfo.message.key.fromMe ?
    ...
    isOwner(senderJid) {
        return senderJid === this.getOwnerJid();
    }

    // Helper to get the owner's JID
    getOwnerJid() {
        return `${config.OWNER_NUMBER}@s.whatsapp.net`;
    }

    // This is a placeholder for processCommand, assuming it exists elsewhere or needs to be defined.
    // For the purpose of this fix, we are modifying how messageInfo.sender is set before calling it.
    async processCommand(messageInfo) {
        // Placeholder for the actual command processing logic
        // The crucial part is that messageInfo.sender is now correctly set for permission checks
        this.logger.debug("Calling processCommand with potentially modified messageInfo.sender");
        // In a real scenario, this would call a method like handleCommand or similar.
