...
class MessageHandler {
    ...
    async process(message) {
        try {
            ...
            console.log(`📝 Full message structure:`, JSON.stringify(message.message, null, 2));
            ...
            for (const type of messageTypes) {
                ...
                if ((type === 'imageMessage' || type === 'videoMessage' || type === 'documentMessage') && content?.caption) {
                    ...
                    console.log(`📷 FOUND MEDIA CAPTION (${type}): "${text}"`);
                    break;
                }
            }
                ...
                for (const type of messageTypes) {
                    ...
                    if (typeof content === 'string') {
                        ...
                        if (editedContent.editedMessage && editedContent.editedMessage.message) {
                            ...
                            console.log('🔍 Found double-nested editedMessage, extracting deeper level');
                        }
            ...
            if (!hasExactPrefix && !hasSpacedPrefix) {
                console.log(`📝 Not a command - text: "${trimmedText}", prefix: "${prefix}"`);
                return;
            }
            ...
            if (messageTypes.includes('editedMessage')) {
                ...
                for (const [topType, topContent] of Object.entries(message.message)) {
                    if (topType !== 'editedMessage' && topContent?.contextInfo) {
                        ...
                        console.log(`🔗 Found ORIGINAL contextInfo in top-level ${topType}`);
                        break;
                    }
                }
                ...
                if (editedContent) {
                    ...
                    if (!contextInfo && message.message.editedMessage?.contextInfo) {
                        ...
                        console.log('🔗 Found contextInfo at editedMessage level');
                    }
                    ...
                    if (!contextInfo) {
                        console.log('🔍 Searching for contextInfo in stored message data...');
                        ...
                        if (messageKey && this.database) {
                            ...
                            if (archivedMessage && archivedMessage.contextInfo) {
                                try {
                                    ...
                                    console.log('🔗 Restored contextInfo from stored message data');
                                } catch (parseError) {
                            ...
                            if (!contextInfo) {
                                console.log('🔍 Using enhanced contextInfo recovery for edited messages...');
                                try {
                                    contextInfo = await this.database.findContextInfoForEditedMessage(messageKey, jids.chat_jid);
                                    if (contextInfo) {
                                        console.log('🔗 Successfully restored contextInfo using enhanced recovery method');
                                    } else {
                                        console.log('🔍 Fallback: Searching recent messages for contextInfo patterns...');
                                            ...
                                            if (recentMsg.contextInfo && recentMsg.id !== messageKey) {
                                                ...
                                                if (timeDiff < timeWindow) {
                                                    try {
                                                        ...
                                                        console.log('🔗 Restored contextInfo from nearby message with similar timestamp');
                                                        break;
                                                    } catch (parseError) {
                        ...
                        if (!contextInfo) {
                            console.log('❌ No contextInfo found in stored data - reply context lost during editing');
                        }
                    }
