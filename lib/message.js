        ...
        try {
            ...
            if (messageTypes.includes('editedMessage')) {
                ...
                if (editedContent) {
                    ...
                    if (!contextInfo) {
                        // Try to find contextInfo from our backup system for edited messages
                        const messageKey = message.key?.id;
                        if (messageKey && this.database) {
                            // First try to find it in the contextInfo restoration system
                            const restoredContextInfo = await this.database.findContextInfoForEditedMessage(messageKey, jids.chat_jid);
                            if (restoredContextInfo) {
