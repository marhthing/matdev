...
const { ZushiPlugin } = require('../plugins/zushi');
...
class MessageHandler {
    ...
    async process(message) {
        // Timing: Start of message processing
        const t0 = Date.now();
        ...
        try {
            ...
            // PRIORITY: Handle media messages with captions first (like imageMessage with .sticker caption)
            
            for (const type of messageTypes) {
                const content = message.message[type];
                
                // Check media types with captions first (imageMessage, videoMessage, documentMessage)
            ...
            if (!text) {
                for (const type of messageTypes) {
                    ...
                    if (typeof content === 'string') {
                        ...
                        // Handle edited messages - extract from deeply nested message structure
                        let editedContent = content.message;
                        
                        // Handle double-nested editedMessage structure from bot.js processing
            ...
            // Check if it's a command (allow any whitespace after prefix)
            ...
            // Extract everything after the prefix and trim leading whitespace
            const afterPrefix = trimmedText.slice(prefix.length).trimStart();
            
            if (!afterPrefix) {
                return; // Just prefix with no command
            }
            
            // this.logger.info(`⚡ Command: ${afterPrefix.split(' ')[0]} from ${jids.is_group ? 'group' : 'private'}`);

            // Parse command and args
            const args = afterPrefix.split(' ').filter(arg => arg.length > 0);
            const commandName = args.shift().toLowerCase();
            
            // Reconstruct command text for compatibility with existing code
            const commandText = prefix + afterPrefix;
            ...
            // Backup contextInfo before archiving (in case message gets edited later)
            await this.database.backupContextInfoForEditing(message);
            
            // Archive the message (handled by JSON storage)
            await this.database.archiveMessage(message);

            // For edited messages, preserve the original contextInfo structure and fix structure issues
            let processedMessage = message.message;
            if (messageTypes.includes('editedMessage')) {
                // Store the original message structure for context preservation
                let originalContextInfo = null;
                
                // First, try to find contextInfo in the TOP level message (before editing)
                for (const [topType, topContent] of Object.entries(message.message)) {
                    if (topType !== 'editedMessage' && topContent?.contextInfo) {
                        originalContextInfo = topContent.contextInfo;
                        break;
                    }
                }
                
                // Navigate through potentially nested editedMessage structure
                let editedContent = message.message.editedMessage?.message;
                
                // Handle double-nested structure from bot.js
                if (editedContent?.editedMessage?.message) {
                    editedContent = editedContent.editedMessage.message;
                }
                
                if (editedContent) {
                    // If we didn't find original context, try to find it in edited content
                    let contextInfo = originalContextInfo;
                    if (!contextInfo) {
                    ...
                    // Also check if contextInfo exists at the editedMessage level
                    if (!contextInfo && message.message.editedMessage?.contextInfo) {
                        contextInfo = message.message.editedMessage.contextInfo;
                    }
                    
                    // ENHANCED: Check for contextInfo in the original message before editing
                    // This handles cases where the original message (like extendedTextMessage) had contextInfo
                    // but it got lost during the editing process
                    if (!contextInfo) {
                        
                        const messageKey = message.key?.id;
                        if (messageKey && this.database) {
                            // First try: Check if we have the contextInfo stored for this exact message ID
                            const archivedMessage = await this.database.getArchivedMessage(messageKey);
                            if (archivedMessage && archivedMessage.contextInfo) {
                                try {
                                    contextInfo = JSON.parse(archivedMessage.contextInfo);
                                } catch (parseError) {
                                    console.log('⚠️ Error parsing stored contextInfo:', parseError.message);
                                }
                            }
                            
                            // Second try: Use enhanced contextInfo finder for edited messages
                            if (!contextInfo) {
                                try {
                                    contextInfo = await this.database.findContextInfoForEditedMessage(messageKey, jids.chat_jid);
                                    if (!contextInfo) {
                                        const recentMessages = await this.database.getRecentMessages(jids.chat_jid, 20);
                                        
                                        // Find a message sent around the same time that might have contextInfo
                                        const currentTime = message.messageTimestamp || Date.now();
                                        const timeWindow = 60000; // 1 minute window
                                        
                                        for (const recentMsg of recentMessages) {
                                            if (recentMsg.contextInfo && recentMsg.id !== messageKey) {
                                                const timeDiff = Math.abs(currentTime - recentMsg.timestamp);
                                                if (timeDiff < timeWindow) {
                                                    try {
                                                        const restoredContextInfo = JSON.parse(recentMsg.contextInfo);
                                                        contextInfo = restoredContextInfo;
                                                        break;
                                                    } catch (parseError) {
                                                        console.log('⚠️ Error parsing nearby contextInfo:', parseError.message);
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    console.log('⚠️ Error in enhanced contextInfo recovery:', error.message);
                                }
                            }
                        }
                    ...
                    // Create proper structure for sticker command to work with edited messages
                    if (contextInfo) {
                        processedMessage = {
                            extendedTextMessage: {
                                text: commandText,
                        ...
                        // Fallback if no contextInfo could be restored
                        if (!processedMessage.extendedTextMessage) {
                            processedMessage = {
                                ...message.message,
                                conversation: commandText
                            };
                            console.log('📝 Added conversation to edited message structure (no contextInfo found)');
                        }
                    }
                }
            }
            ...
            // Timing: Before permission check
            const t1 = Date.now();
            const hasPermission = await this.checkPermissions(commandName, jids.participant_jid, jids.from_me, messageInfo);
            // Timing: After permission check
            const t2 = Date.now();
            if (!hasPermission) {
                return;
            }
            ...
            const t4 = Date.now();
            // this.logger.info(`⏱️ Timing for ${commandName}: total=${t4-t0}ms | prePerm=${t1-t0}ms | perm=${t2-t1}ms | preExec=${t3-t2}ms | exec=${t4-t3}ms`);
     ...
     */
    async checkPermissions(commandName, participantJid, fromMe, messageInfo = null) {
        try {
            const config = require('../config');
            ...
            // Check group-wide zushi permission (if in group, before user permission)
            // Use messageInfo if provided, otherwise fallback to no group-wide check
            let isGroup = false, chatJid = null;
            if (messageInfo && typeof messageInfo === 'object') {
                isGroup = messageInfo.is_group;
                chatJid = messageInfo.chat_jid;
            }
            // If messageInfo is not provided, try to infer from participantJid (not always reliable)
            if (!isGroup && participantJid && participantJid.endsWith('@g.us')) {
                isGroup = true;
                chatJid = participantJid;
            }
            if (this.bot && this.bot.plugins && isGroup && chatJid) {
                if (ZushiPlugin.isGroupCommandAllowed(this.bot, chatJid, commandName)) {
                    return true;
                }
            }
    ...
    async processStickerCommand(message, jids) {
        try {
            ...
            const hasPermission = await this.checkPermissions(boundCommand.command, jids.participant_jid, jids.from_me, messageInfo);
