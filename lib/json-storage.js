...
class JSONStorageManager {
    ...
    async archiveMessage(message) {
        try {
            ...
            // Enhanced participant extraction for new WhatsApp LID format
            let participant = sender;
            if (isGroup || isStatus) {
                if (message.key.participant) {
                    // Check if it's LID format and convert to standard format
            ...
            if (message.key.fromMe) {
                // For outgoing messages, the sender is ALWAYS the bot/owner
                const botNumber = this.bot?.sock?.user?.id?.split(':')[0];
                if (botNumber) {
                    actualSender = `${botNumber}@s.whatsapp.net`;
                    actualParticipant = actualSender;
                }
                // Keep chat_jid as the group/private chat JID
            } else {
                // For incoming messages, ensure sender_jid is the actual sender
                if (isGroup || isStatus) {
                    // In groups/status, participant is the actual sender (now properly converted from LID)
