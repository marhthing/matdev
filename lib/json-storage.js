...
class JSONStorageManager {
    ...
    async findContextInfoForEditedMessage(editedMessageId, chatJid) {
        try {
            ...
            // Strategy 2: Look for a message that has contextInfo AND was sent recently from the same chat
            ...
            // Find the most recent message with contextInfo that could be the original
            ...
            // Strategy 3: If we know the original message key, find contextInfo that references it
            const messagesReferencingOriginal = messagesArray
                .filter(msg => 
                    msg.chat_jid === chatJid && 
                    msg.contextInfo &&
                    msg.originalMessageKey
                )
                .sort((a, b) => b.timestamp - a.timestamp);
            
            if (messagesReferencingOriginal.length > 0) {
                try {
                    const contextInfo = JSON.parse(messagesReferencingOriginal[0].contextInfo);
                    this.logger.debug(`🔍 Found contextInfo via original message key relationship`);
                    return contextInfo;
                } catch (parseError) {
                    this.logger.debug('Error parsing contextInfo from relationship:', parseError.message);
                }
            }
