    ...
    async archiveMessage(message) {
        try {
            ...
            if (mediaTypes.includes(messageType)) {
                ...
                this.logger.info(`⚡ INSTANT archive: ${messageType} ${isVoiceNote ? '(voice note)' : ''} - downloading in background`);
                ...
                this.downloadAndSaveMedia(message, messageType, isStatus)
                    .then(downloadResult => {
                        if (downloadResult) {
                            ...
                            this.logger.success(`✅ Background download complete: ${downloadResult.filename}`);
                        }
                    })
                    .catch(mediaError => {
