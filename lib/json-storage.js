    ...
    async archiveMessage(message) {
        try {
            ...
            if (messageTypes.length > 1) {
                ...
                this.logger.info(`📱 Complex message with types: [${messageTypes.join(', ')}] - processing as: ${messageType}`);
            }
            
            // Enhanced text extraction with fallbacks
            let text = this.extractTextFromMessage(message, messageType, content);
            
            this.logger.debug(`Message type: ${messageType}, hasText: ${!!text}, messageTypes: [${messageTypes.join(', ')}]`);
            ...
            // Log enhanced message info for debugging
            this.logger.info(`📝 Archiving message: type=${messageType}, hasMedia=${hasMedia}, hasText=${hasText}, isVoiceNote=${isVoiceNote}`);
            ...
            // Enhanced media handling with voice note detection
            const mediaTypes = ['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'];
            if (mediaTypes.includes(messageType)) {
                // Enhanced logging for different media types
                if (messageType === 'audioMessage') {
                    this.logger.info(`🎵 Processing ${isVoiceNote ? 'voice note' : 'audio message'}:`, {
                ...
                try {
                    ...
                    if (downloadResult) {
                        ...
                        this.logger.success(`✅ Successfully archived ${messageType} - ${downloadResult.filename}`);
                    } else {
                        this.logger.warn(`⚠️ Failed to download ${messageType} but continuing with text archival`);
                    }
                } catch (mediaError) {
