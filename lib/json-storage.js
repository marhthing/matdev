...
class JSONStorageManager {
    ...
    async archiveMessage(message) {
        try {
            ...
            // Skip newsletter messages without meaningful content
            if (isNewsletter && (!text || text.trim().length === 0)) {
            ...
            // Skip messages with no meaningful content (empty text and no media)
            ...
            // Enhanced participant extraction - preserve original LID format
            let participant = sender;
            if (isGroup || isStatus) {
                if (message.key.participant) {
                    participant = message.key.participant; // Keep original format (including @lid)
                } else if (message.key.participantPn) {
                    participant = message.key.participantPn;
                } else if (message.key.participantLid) {
                    participant = message.key.participantLid; // Keep original LID format
                }
            }
            ...
            if (message.key.fromMe) {
                ...
                if (isGroup || isStatus) {
                    // In groups/status, participant is the actual sender (preserve original format)
                    actualSender = participant;
                } else {
                    // In private chats, sender (remoteJid) is the actual sender
                    actualSender = sender;
                    actualParticipant = sender;
                }
            }
