...
class JSONStorageManager {
    ...
    async initialize() {
        try {
            ...
            // Note: Cleanup is now handled by centralized CleanupManager
     ...
     * NOTE: This is now disabled - cleanup is handled by centralized CleanupManager
     */
    startCleanupScheduler() {
        // Cleanup is now handled centrally by CleanupManager
        this.logger.info('🕐 Cleanup scheduler disabled - using centralized CleanupManager');
    }
    ...
    async loadMessages() {
        try {
            if (await fs.pathExists(this.messagesFile)) {
                ...
                if (!fileContent.trim()) {
                    this.logger.info('Messages file is empty, initializing with empty storage');
                    this.messages = new Map();
    ...
    async loadPermissions() {
        try {
            if (await fs.pathExists(this.permissionsFile)) {
                const fileContent = await fs.readFile(this.permissionsFile, 'utf8');
            ...
            this.logger.warn('Error loading permissions, starting fresh:', error.message);
            this.permissions = new Map();
            await this.savePermissions();
        }
    }
    ...
    async loadGroupLid() {
        try {
            if (await fs.pathExists(this.groupLidFile)) {
                const fileContent = await fs.readFile(this.groupLidFile, 'utf8');
                ...
                this.groupLidData = JSON.parse(fileContent);
            } else {
                this.groupLidData = null;
            }
        } catch (error) {
            this.logger.warn('Error loading group LID data, starting fresh:', error.message);
            this.groupLidData = null;
            await this.saveGroupLid();
        }
    }
    ...
    async loadEditedMessages() {
        try {
            if (await fs.pathExists(this.editedMessagesFile)) {
                const fileContent = await fs.readFile(this.editedMessagesFile, 'utf8');
            ...
            this.logger.warn('Error loading edited messages, starting fresh:', error.message);
            this.editedMessageMap = new Map();
            await this.saveEditedMessages();
        }
    }
    ...
    async loadOriginalContextInfo() {
        try {
            ...
            if (await fs.pathExists(contextInfoFile)) {
                const fileContent = await fs.readFile(contextInfoFile, 'utf8');
            ...
            this.logger.warn('Error loading original contextInfo, starting fresh:', error.message);
            this.originalContextInfoMap = new Map();
            await this.saveOriginalContextInfo();
        }
    }
    ...
    async archiveMessage(message) {
        try {
            ...
            if (!hasMedia && !hasText && messageType !== 'protocolMessage') {
                // Original debug log that was causing terminal clutter
                if (!text || text.trim().length === 0) {
                    // console.log(`No text content found in message ${message.key.id}. Available types:`, messageTypes.join(', '));
                }
            ...
            if (messageTypes.includes('editedMessage')) {
                ...
                if (contextInfoStr) {
                    // Store with timestamp for proper cleanup later
                    this.originalContextInfoMap.set(message.key.id, {
                        contextInfo: contextInfoStr,
                        timestamp: Date.now()
                    });
                    await this.saveOriginalContextInfo();
                    this.logger.debug(`💾 Backup contextInfo for edited message ${message.key.id} with timestamp`);
                }
            }
    ...
    async cleanupOldContextInfo() {
        try {
            ...
            // Only clean up if we have contextInfo entries and they have proper timestamp structure
            for (const [messageId, contextData] of this.originalContextInfoMap.entries()) {
                let shouldDelete = false;
                
                // Check if contextData has timestamp (new format: {contextInfo: ..., timestamp: ...})
                if (contextData && typeof contextData === 'object' && contextData.timestamp) {
                    if (now - contextData.timestamp > retentionPeriod) {
                        keysToDelete.push(messageId);
                        shouldDelete = true;
                    }
                }
                // For legacy format (just string/object without timestamp), keep them but log
                else if (contextData) {
                    this.logger.debug(`Keeping contextInfo backup for ${messageId} (no timestamp available)`);
                }
            ...
            // Only delete entries that actually should be deleted based on age
            for (const messageId of keysToDelete) {
                this.originalContextInfoMap.delete(messageId);
            }

            // Save the updated map if we made changes
            if (keysToDelete.length > 0) {
                await this.saveOriginalContextInfo();
                this.logger.info(`🗑️ Cleaned up ${cleanedCount} old contextInfo backups (older than 7 days).`);
            } else if (this.originalContextInfoMap.size > 0) {
                this.logger.debug(`📋 Keeping ${this.originalContextInfoMap.size} contextInfo backups (no cleanup needed).`);
            }
    ...
    async backupContextInfoForEditing(message) {
        try {
            ...
            if (contextInfoStr) {
                // Store with timestamp for proper cleanup later
                this.originalContextInfoMap.set(messageId, {
                    contextInfo: contextInfoStr,
                    timestamp: Date.now()
                });
                await this.saveOriginalContextInfo();
