...
class DatabaseManager {
    ...
    async archiveMessage(message) {
        ...
        try {
            ...
            const sql = `
                ...
                (id, chat_jid, sender_jid, participant_jid, message_type, content, timestamp)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `;
    ...
    async cleanupOldMessages() {
        ...
        return new Promise((resolve) => {
            this.db.run(sql, [thirtyDaysAgo], function(error) {
                if (!error) {
                    console.log(`🗑️ Cleaned up ${this.changes} old messages`);
                }
                resolve(!error);
            });
        });
