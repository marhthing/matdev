...
class WaifuPlugin {
    ...
    async sendWaifu(messageInfo, waifuData, category = null, counter = null) {
        try {
            let caption = `✨ *Generated Waifu*${counter ? ` (${counter})` : ''}\n\n`;
            
            if (category) {
                caption += `🏷️ *Category:* ${category}\n`;
            }
            
            if (waifuData.tags && waifuData.tags.length > 0) {
                caption += `🎨 *Tags:* ${waifuData.tags.join(', ')}\n`;
            }
            
            caption += `👨‍🎨 *Artist:* ${waifuData.artist}\n`;
            caption += `🔗 *Source:* ${waifuData.source}\n\n`;
            caption += `_Generated by MATDEV Waifu Generator_ 🤖✨`;
