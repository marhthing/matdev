...
<body>
    ...
    <div class="footer">Generated by MATDEV Bot - ${new Date().toLocaleString()}</div>
            ...
            // Ultimate fallback: Create a proper document file instead of fake PDF
            const docFileName = fileName.replace('.pdf', '.txt');
            const docFilePath = path.join(__dirname, '..', 'tmp', docFileName);
...
${new Date().toLocaleString()}
            ...
            await fs.writeFile(docFilePath, formattedContent, 'utf8');

            return {
                success: true,
                filePath: docFilePath,
                fileName: docFileName.replace('.txt', '_document.txt')
            };
